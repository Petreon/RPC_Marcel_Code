...
// functions
void Derivada_H(double B, double L, double H, Eigen::MatrixXd rpcNumerator, Eigen::MatrixXd rpcDenominator) {
/* Reason to why used rpc_matrix.row(x).value() instead index[] is because Eigen can do more runtime errorproof in memory
   ...
   // doing this operation by parts, every part should return a double value:
    ...
    double part1 = (3*rpcNumerator.row(19).value() * std::pow(H,2) + 2*rpcNumerator.row(13).value() * H * L + 2*rpcNumerator.row(16).value() * H * B + 2*rpcNumerator.row(9).value() * H + rpcNumerator.row(17).value() * std::pow(L,2) + rpcNumerator.row(10).value() * L * B + rpcNumerator.row(5).value() * L + rpcNumerator.row(18).value() * std:pow(B,2) + rpcNumerator.row(6).value() * B + rpcNumerator.row(3).value());
    ...
    */
    ...
    double part3 = std::pow(part2, 2);

    double result = part1 / part2 - part1 * part2 / part3;
